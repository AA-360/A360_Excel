...
//    }
    ...
    public void save(String inputFile){
        try (FileOutputStream out = new FileOutputStream(inputFile)) {
            ...
            throw new BotCommandException("Error saving the file: " + ex.getMessage());
        }
    }

    public Cell getCell(Sheet sht, CellAddress ca){
        //CellReference cr = new CellReference(address);
        Integer idxRow = ca.getRow();
        Integer idxCol = ca.getColumn();
        try {
            Row rw = sht.getRow(idxRow);
            if (rw == null) return null;
            Cell cl = rw.getCell(ca.getColumn());
            return cl;
        }
        catch (Exception e){
            Row r = null;
            for(Row rw : sht){
                if(rw.getRowNum() == idxRow){
                    r = rw;
                    break;
                }
                if(rw.getRowNum() > idxRow){
